...
def end_of_epoch_metrics(args, model, data_loader, device, logger):
    ...
    bpp_total, q_bpp_total, q_bpp_total_attained, LPIPS_total = torch.Tensor(N), torch.Tensor(N), torch.Tensor(N), torch.Tensor(N)
    ...
    with torch.no_grad():
        ...
        for idx1, (dataAll, yAll) in enumerate(tqdm(data_loader), 0):
          ...
          for idxB in range(B):
            ...
            q_bpp_attained = compressed_output.total_bpp_attained
    ...
    logger.info(f'BPPMean: {q_bpp_total.std(dim=0):.3f}')
