...
from tensorflow.keras.utils.to_categorical as to_categorical
# Custom modules
    ...
    with torch.no_grad():
        ...
        for idx, (data, bpp, filenames) in enumerate(tqdm(eval_loader), 0):
            ...
            if args.reconstruct is True:
                ...
                # Perform entropy coding
                compressed_output, latOutPre = model.compressDump(data)
            ...
            for subidx in range(reconstruction.shape[0]):
                ...
                np.savez(fnameNPX,lat0=thisLat0, lat1=thisLat1, hyp1=thisHyp1, thisCat = to_categorical(args.thisCat,101))
