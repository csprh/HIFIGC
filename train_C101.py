...
def end_of_epoch_metrics(args, model, data_loader, device, logger):
    ...
    with torch.no_grad():
        ...
        for idx, (data, y) in enumerate(tqdm(data_loader), 0):
            ...
            for subidx in range(reconstruction.shape[0]):
                ...
                fname = os.path.join(args.output_dir, "{}_RECON_{:.3f}bpp.png".format(idx, q_bpp_per_im))
            ...
            comp_loss_total[n:n + B] = compression_loss.data
            if model.use_classiOnly is True:
                classi_loss_total[n:n + B] = classi_loss.data
            n += B
